...
WheelSession.prototype.displayWheel = function (pointer) {
// Modified from https://github.com/JavoByte/rouletteWheel/demo.html 
    ...
    try { audioElement = document.createElement("audio");
          audioElement.setAttribute("src", "/mp3/prize_wheel.mp3");
    } catch (err) { console.log("Error in audio:", err);
    }

    var roulettePars = {
      pointer: $('<img>').attr('src', pointer)[0],
      items : items,
        start : function() {
...
Qwheel.readyAux = function (pointer) {
   curSession.start();
   curSession.displayWheel(pointer);
}
...
function showSessions(sessionID) {
    ...
    $('#stateSelect').change(function() {
       ...
       var newSessionID = '';
       if (selState == "NEW") {
           selState = window.prompt('Enter new session name (no spaces):');
           if (selState)
...
$(document).ready( ()=>{Qwheel.readyAux('/null-lock/img/pointer.png')});